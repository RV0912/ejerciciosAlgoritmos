...
public class Ejercicio_49_RV {
	...
	public static void cantidadDeAnimales(int perro, int gato) {
		System.out.println("\nCant. de perros: " + perro);
		System.out.println("\nCant. de gatos: " + gato);
	}

	public static void main(String[] args) {
		ingresarNombre();
		System.out.println("\nTrabajo?(escriba si o no): ");
		siono = ingreso.nextLine();

		flag = analizarSiTrabaja(siono);

		float alt = almacenarAltura(altura);
		System.out.println("\nAltura: " + alt);
		System.out.println("\nEdad: " + filtrarEdad(edadFiltrada));
		
		if (flag == true) {
			System.out.println("\ntrabajooooo ☺☺☺☺☺☺");
		} else {
			System.err.println("\ncomo q no trabajaste vago☻☻☻☻(no hay carita triste)");
		}
		cantidadDeAnimales(1, 0);
